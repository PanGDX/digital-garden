...
\newcommand{\D}[1]{\,\mathrm{d}#1} % Added to fix undefined \D{x}
...
\ssmall
\begin{multicols*}{4}
  \header % Placed here to output safely at the top of the first column
  \raggedcolumns
  ...
  \begin{itemize}
    ...
    \item If $Y_1,Y_2,...,Y_k$ are \textbf{independent} chi-square RV with $n_1,n_2,...,n_k$ degrees of freedom, then $Y_1+Y_2+...+Y_k$ has a chi-square distr with $n_1+n_2+...+n_k$ degrees of freedom: $\sum_{i=1}^k Y_i \sim \chi^2(\sum_{i=1}^k n_i)$
